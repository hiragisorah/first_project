...
void Seed::NodeSystem::SimpleNodeSystem::Initialize(void)
{
	...
	auto scale = this->graphics_->GetShaderVariable<float2>(this->repeat_shader_, "g_scale");
	auto scroll = this->graphics_->GetShaderVariable<float2>(this->repeat_shader_, "g_scroll");
}
